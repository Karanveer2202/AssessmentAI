...
class RotaryPositionalEmbedding(nn.Module):
    def __init__(self, hidden_size, max_seq_len):
        ...
        self.sinusoid_table = nn.Parameter(torch.zeros(max_seq_len, hidden_size))
        self.cosinusoid_table = nn.Parameter(torch.zeros(max_seq_len, hidden_size))
    ...
    def forward(self, x):
        seq_len = x.size(1)
        sinusoid_table = self.sinusoid_table[:seq_len, :]
        cosinusoid_table = self.cosinusoid_table[:seq_len, :]

        x = x * sinusoid_table + x * cosinusoid_table
        return x
...
class GPT2(nn.Module):
    def __init__(self, num_layers, hidden_size, num_heads, vocab_size, max_seq_len, attention_dropout=0.1, feed_forward_dropout=0.1):
        ...
        self.positional_embedding = RotaryPositionalEmbedding(hidden_size, max_seq_len)
    ...
    def forward(self, x, mask=None):
        out = self.token_embedding(x) + self.positional_embedding(x)
